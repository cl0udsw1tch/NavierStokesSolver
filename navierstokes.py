...
class NavierStokesSolver:
    ...
    def __init__(
        ...
        if not n_rows or not n_cols:
            ...
            self.n_rows     = n_rows
            self.n_cols     = n_cols
        self._constructor_args_['n_rows'] = self.n_rows
        self._constructor_args_['n_cols'] = self.n_cols
        ...
        self._acc_order = 2 if scheme == 'central_difference' else 1.5 if scheme == 'hybrid' else 1
    ...
    def solve(self, objective: Literal['\phi', 'm'] ='\phi', solveSparse=True, tolerance=0.01, max_iter=10_000, threshold=1e-2):
        
        if objective == '\phi':
            self._construct_problem_(objective='\phi')
            _, e = self._solve_jacobi_(objective='\phi', tolerance=tolerance, max_iter=max_iter) if not solveSparse else self._solve_sparse_(objective='\phi')
        ...
        while err > threshold * self.delta_x**self._acc_order:
            ...
            p_err = np.abs(self.x['p\'']).mean()
            u_err = np.abs(self.x['u'] - u_prev).mean()
            v_err = np.abs(self.x['v'] - v_prev).mean()
            
            err = u_err + v_err + p_err
            self.error = err
            print(f"\rIteration: {i}, u_err: {u_err:.3e}, v_err: {v_err:.3e}, p'_err: {p_err:.3e}", sep='', end='')
    ...
    def plot(self, 
        ...
        # plot obstacles by finding nans in self.x[objective]
        nans = np.where(np.isnan(core) if objective is not None else np.isnan(self.x['u'][1:-1, 1:-1][::-1]))
        for i in range(len(nans[0])):
            ax.plot(nans[1][i], nans[0][i], 'ks')
    ...
    def order_conv(self, 
        ...
        if objective == '\phi':
            ...
            for i in range(3):
                ...
                o[i] = self._get_order_(x_coarse[1:-1, 1:-1], x_fine[1:-1, 1:-1], x_exact[1:-1, 1:-1], coarse_res, fine_res, exact_res, objective=['u', 'v', 'p'][i])
            return o
    ...
    def _obstacle_coords_(self, obstacle_coords):
        ...
        pixel_x1 = int(x1 * self.n_cols)
        pixel_y1 = (self.n_rows) - int(y2*self.n_rows)
        pixel_x2 = int(x2 * self.n_cols)
        pixel_y2 = (self.n_rows) - int(y1*self.n_rows)
        
        return (pixel_x1, pixel_y1), (pixel_x2, pixel_y2)
    

    def _get_order_(self, x_coarse, x_fine, x_exact, coarse_res, fine_res, exact_res, objective='\phi') -> float:
        linsp_coarse = np.linspace(0, 1, coarse_res)
        linsp_fine = np.linspace(0, 1, fine_res)
        linsp_exact = np.linspace(0, 1, exact_res)

        # Convert solutions to masked arrays, masking np.nan values
        x_coarse_masked = np.ma.masked_invalid(x_coarse)
        x_fine_masked = np.ma.masked_invalid(x_fine)
        x_exact_masked = np.ma.masked_invalid(x_exact)

        # Create grids for interpolation
        coarse_grid = np.array(np.meshgrid(linsp_coarse, linsp_coarse)).T.reshape(-1, 2)
        fine_grid = np.array(np.meshgrid(linsp_fine, linsp_fine)).T.reshape(-1, 2)
        exact_grid = np.array(np.meshgrid(linsp_exact, linsp_exact)).T.reshape(-1, 2)

        # Extract valid data points and their coordinates
        coarse_points = coarse_grid[~x_coarse_masked.mask.ravel()]
        coarse_values = x_coarse_masked.compressed()

        fine_points = fine_grid[~x_fine_masked.mask.ravel()]
        fine_values = x_fine_masked.compressed()

        # Interpolate coarse and fine solutions to the exact grid
        x_coarse_interp = griddata(
            coarse_points,
            coarse_values,
            exact_grid,
            method='cubic',
            fill_value=np.nan  # Fill outside the convex hull with np.nan
        ).reshape(exact_res, exact_res)

        x_fine_interp = griddata(
            fine_points,
            fine_values,
            exact_grid,
            method='cubic',
        ...
        # Mask the interpolated solutions to match the exact solution's mask
        x_coarse_interp_masked = np.ma.masked_invalid(x_coarse_interp)
        x_fine_interp_masked = np.ma.masked_invalid(x_fine_interp)

        # Calculate absolute errors (L2 norm) using masked arrays
        err_coarse = np.ma.sqrt(np.ma.mean((x_exact_masked - x_coarse_interp_masked) ** 2))
        err_fine = np.ma.sqrt(np.ma.mean((x_exact_masked - x_fine_interp_masked) ** 2))

        O = np.ma.log(err_coarse / err_fine) / np.ma.log(fine_res / coarse_res)

        fig, ax = plt.subplots(nrows=1, ncols=3, figsize=(20, 5))
        x_points = np.arange(0, exact_res)
        y_points = np.arange(0, exact_res)
        X, Y = np.meshgrid(x_points, y_points)

        core = x_exact[::-1]
        masked_core = np.ma.masked_where(core == np.nan, core)
        contour = ax[0].contourf(X, Y, masked_core, levels=50, corner_mask=False, cmap='jet')
        fig.colorbar(contour, ax=ax[0]).ax.set_title(label=f'${objective}$')
        ax[0].set_title(f"{exact_res}x{exact_res} Grid (Exact)")

        core = x_coarse_interp_masked[::-1]
        masked_core = np.ma.masked_where(core == np.nan, core)
        contour = ax[1].contourf(X, Y, masked_core, levels=50, corner_mask=False, cmap='jet')
        fig.colorbar(contour, ax=ax[1]).ax.set_title(label=f'${objective}$')
        ax[1].set_title(f"{coarse_res}x{coarse_res} Grid (coarse, interpolated)")

        core = x_fine_interp_masked[::-1]
        masked_core = np.ma.masked_where(core == np.nan, core)
        contour = ax[2].contourf(X, Y, masked_core, levels=50, corner_mask=False, cmap='jet')
        fig.colorbar(contour, ax=ax[2]).ax.set_title(label=f'${objective}$')
        ax[2].set_title(f"{fine_res}x{fine_res} Grid (fine, interpolated)")

        num_ticks = 5
        x_ticks = np.linspace(0, exact_res-1, num_ticks)
        x_labels = np.linspace(0, self.L_x, num_ticks)

        y_ticks = np.linspace(0, exact_res-1, num_ticks)
        y_labels = np.linspace(0, self.L_y, num_ticks)

        for _ax in ax:
            _ax.set_xticks(x_ticks)
            _ax.set_xticklabels(x_labels, rotation=0)
            _ax.set_yticks(y_ticks)
            _ax.set_yticklabels(y_labels, rotation=0)
            _ax.margins(x=0,y=0)

        for obstacle in self.obstacles:
            (x1,y1),(x2,y2) = obstacle
            for _ax in ax:
                _ax.add_patch(Rectangle((x1, self.n_rows - y2), x2-x1-1/2, y2-y1-1/2,  fill='white', hatch='..', zorder=20))

        plt.suptitle(f"Intepolated Plots for ${objective}$ with Order of Convergence: {O:.3f}")
        plt.show()
        return O
    ...
    def _solve_sparse_(self, objective: Literal["\phi", "m", "p\'"] = "\phi") -> Tuple[np.ndarray, float] :
        ...
        x = scipy.sparse.linalg.spsolve(acsc, bflat)
        res = -1
    ...
    def _scalar_coeff_hybrid_(self, A_p, A_e, A_w, A_n, A_s, objective='\phi'):
        ...
        A_e[self.has_e_bnd_mask]                                    = 0
        b_e[np.roll(self.dirichlet_bnd[objective]['e'], -1, 1)]     = np.maximum(2 * self.D['e'] - F_x[self.dirichlet_bnd[objective]['e'][:, 1:]], np.maximum(0, -F_x[self.dirichlet_bnd[objective]['e'][:, 1:]]))
        b_e[np.roll(self.neumann_bnd[objective]['e'], -1, 1)]       = 0
        
        A_w[self.has_w_nbr_mask]                                    = np.maximum(self.D['w'] + (1/2) * F_x[self.u_f_fluid_mask], np.maximum(0, F_x[self.u_f_fluid_mask]))
        A_w[self.has_w_bnd_mask]                                    = 0
        b_w[np.roll(self.dirichlet_bnd[objective]['w'], 1, 1)]      = np.maximum(2 * self.D['w'] + F_x[self.dirichlet_bnd[objective]['w'][:, :-1]], np.maximum(0, F_x[self.dirichlet_bnd[objective]['w'][:, :-1]]))
        b_w[np.roll(self.neumann_bnd[objective]['w'], 1, 1)]        = 0
        
        A_n[self.has_n_nbr_mask]                                    = np.maximum(self.D['n'] - (1/2) * F_y[self.v_f_fluid_mask], np.maximum(0, -F_y[self.v_f_fluid_mask]))
        A_n[self.has_n_bnd_mask]                                    = 0
        b_n[np.roll(self.dirichlet_bnd[objective]['n'], 1, 0)]      = np.maximum(2 * self.D['n'] - F_y[self.dirichlet_bnd[objective]['n'][:-1, :]], np.maximum(0, -F_y[self.dirichlet_bnd[objective]['n'][:-1, :]]))
        b_n[np.roll(self.neumann_bnd[objective]['n'], 1, 0)]        = 0
        
        A_s[self.has_s_nbr_mask]                                    = np.maximum(self.D['s'] + (1/2) * F_y[self.v_f_fluid_mask], np.maximum(0, F_y[self.v_f_fluid_mask]))
        A_s[self.has_s_bnd_mask]                                    = 0
        b_s[np.roll(self.dirichlet_bnd[objective]['s'], -1, 0)]     = np.maximum(2 * self.D['s'] + F_y[self.dirichlet_bnd[objective]['s'][1:, :]], np.maximum(0, F_y[self.dirichlet_bnd[objective]['s'][1:, :]]))
        b_s[np.roll(self.neumann_bnd[objective]['s'], -1, 0)]       = 0
    ...
    def _p_corr_coeffs_(self):
        ...
        if not np.any(self.dirichlet_bnd['p']['all']):
            self.A['p\'']['p'][tuple(np.argwhere(self.e_nbr_mask)[-1])] += 1e30
            self.b['p\''][tuple(np.argwhere(self.e_nbr_mask)[-1])] = 0
    ...
    def _correct_p_(self):
        ...
        if not np.any(self.dirichlet_bnd['p']['all']):
            self.x['p'][tuple(np.argwhere(self.e_nbr_mask)[-1])] = 0
    ...
    def _init_mass_cont_(self):
        if np.any(self.neumann_bnd['m']['e']):
            fix = (self.influx / (self.neumann_bnd['m']['e'].sum() * self._area_('e')))
            self.x['u'][self.neumann_bnd['m']['e']] = fix
        ...
        elif np.any(self.neumann_bnd['m']['w']):
            fix = (self.influx / (self.neumann_bnd['m']['w'] .sum() * self._area_('w')))
            self.x['u'][self.neumann_bnd['m']['w']] = fix
        ...
        elif np.any(self.neumann_bnd['m']['n']):
            fix = (self.influx / (self.neumann_bnd['m']['n'] .sum() * self._area_('n')))
            self.x['u'][self.neumann_bnd['m']['n']] = self.x['u'][np.roll(self.neumann_bnd['m']['n'], 1, 0)]
            self.x['v'][self.neumann_bnd['m']['n']] = fix
            self.x['v_f'][self.neumann_bnd['m']['n'][:-1, :]] = self.x['v'][self.neumann_bnd['m']['n']]

        elif np.any(self.neumann_bnd['m']['s']):
            fix = (self.influx / (self.neumann_bnd['m']['s'] .sum() * self._area_('s')))
            self.x['u'][self.neumann_bnd['m']['s']] = self.x['u'][np.roll(self.neumann_bnd['m']['s'], -1, 0)]
            self.x['v'][self.neumann_bnd['m']['s']] = fix
    ...
    def _set_homog_neum_uv_(self):
        ...
        if np.any(self.neumann_bnd['m']['e']):
            outflux = (self.x['u'][np.roll(self.neumann_bnd['m']['e'], -1, 1)] ).sum() * self._area_('e')
            fix = (self.influx / outflux)
            self.x['u'][self.neumann_bnd['m']['e']] = self.x['u'][np.roll(self.neumann_bnd['m']['e'], -1, 1)] * fix
            self.x['v'][self.neumann_bnd['m']['e']] = self.x['v'][np.roll(self.neumann_bnd['m']['e'], -1, 1)]
            self.x['u_f'][self.neumann_bnd['m']['e'][:, 1:]] = self.x['u'][self.neumann_bnd['m']['e']]

        elif np.any(self.neumann_bnd['m']['w']):
            outflux = (self.x['u'][np.roll(self.neumann_bnd['m']['w'], 1, 1)] * self._area_('w')).sum()
            fix = (self.influx / outflux )
            self.x['u'][self.neumann_bnd['m']['w']] = self.x['u'][np.roll(self.neumann_bnd['m']['w'], 1, 1)] * fix
            self.x['v'][self.neumann_bnd['m']['w']] = self.x['v'][np.roll(self.neumann_bnd['m']['w'], 1, 1)]
            self.x['u_f'][self.neumann_bnd['m']['w'][:, :-1]] = self.x['u'][self.neumann_bnd['m']['w']]

        elif np.any(self.neumann_bnd['m']['n']):
            outflux = (self.x['v'][np.roll(self.neumann_bnd['m']['n'], 1, 0)] * self._area_('n')).sum()
            fix = (self.influx / outflux) if np.any(self.neumann_bnd['m']['n']) else 0
            self.x['u'][self.neumann_bnd['m']['n']] = self.x['u'][np.roll(self.neumann_bnd['m']['n'], 1, 0)]
            self.x['v'][self.neumann_bnd['m']['n']] = self.x['v'][np.roll(self.neumann_bnd['m']['n'], 1, 0)] * fix
            self.x['v_f'][self.neumann_bnd['m']['n'][:-1, :]] = self.x['v'][self.neumann_bnd['m']['n']]

        elif np.any(self.neumann_bnd['m']['s']):
            outflux = (self.x['v'][np.roll(self.neumann_bnd['m']['s'], -1, 0)] * self._area_('s')).sum()
            fix = (self.influx / outflux)
            self.x['u'][self.neumann_bnd['m']['s']] = self.x['u'][np.roll(self.neumann_bnd['m']['s'], -1, 0)]
            self.x['v'][self.neumann_bnd['m']['s']] = self.x['v'][np.roll(self.neumann_bnd['m']['s'], -1, 0)] * fix
            self.x['v_f'][self.neumann_bnd['m']['s'][1:, :]] = self.x['v'][self.neumann_bnd['m']['s']]
